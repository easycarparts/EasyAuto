...
// Process all batches
for (let batchNum = 8; batchNum <= 15; batchNum++) {
  const skip = (batchNum - 7) * 100;
  console.log(`\nProcessing batch ${batchNum} (skip=${skip})...`);

  // Export batch
  try {
    execSync(`node scripts/export-thin.mjs --limit 100 --skip ${skip}`, { stdio: 'pipe' });
  } catch (e) {
    console.error(`Failed to export batch ${batchNum}:`, e.message);
    continue;
  }
  ...
  let batch;
  try {
    batch = JSON.parse(fs.readFileSync('scripts/_batch.json', 'utf8'));
  } catch (e) {
  ...
  if (badCount === 0) {
    const filename = `batch_${batchNum}_descriptions.json`;
    fs.writeFileSync(filename, JSON.stringify(result, null, 2));
    console.log(`✓ Batch ${batchNum} complete: ${filename}`);
  } else {
    console.log(`✗ Batch ${batchNum}: ${badCount} entries out of range`);
  }
}
